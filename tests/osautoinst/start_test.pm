...
sub full_run {
    ...
    assert_script_run("retry -r 5 -e -- timeout 500 openqa-clone-job --show-progress --from $openqa_url --parental-inheritance \$job_id $pflash_settings", timeout => 900);
}
...
sub full_run_multimachine {
    ...
    assert_script_run("retry -r 5 -e -- timeout 500 openqa-clone-job --show-progress --skip-chained-deps --from $openqa_url --parental-inheritance \$job_id $pflash_settings", timeout => 1800);
}
